...
Each **page** is an object. And should have the following properties:
...
| customActions | `object[]` | false | A list of extra (non RESTful) endpoints available in your ~RESTful~ API. Specifically `customActions` is a list of PUT or POST method objects. <br /><br />For example this could enable an endpoint like: ``PUT /users/:id/disable`` |
...
> If `customActions` is not empty then a drop down appears in the Edit Item dialog which allows users to select the action they want to perform.
