...
| onChange | `function` | false | A custom callback function that is triggered when the field's value changes. Only available when using dynamic (js) config. The function receives the array of all current form fields and the new value of the changed field. Use this to implement field dependencies, such as resetting another field when a field changes to a particular value.<br>Example:` "onChange": (fields, value) => { if (value === "B") { const dependentField = fields.find(f => f.originalName === "dependentField"); if (dependentField) { dependentField.value = null; } } }` |
